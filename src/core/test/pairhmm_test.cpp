...
// 测试 AVX512 Float
void test_avx512_float()
{
    printf("Testing AVX512 Float...\n");
    
    TestCase tc;
    std::vector<uint8_t> hap_data, rs_data, q_data, i_data, d_data, c_data;
    create_simple_test_case(tc, hap_data, rs_data, q_data, i_data, d_data, c_data);
    
    float result = compute_pairhmm_avx512_float(tc);
...
// 测试 AVX512 Double
void test_avx512_double()
{
    printf("Testing AVX512 Double...\n");
    
    TestCase tc;
    std::vector<uint8_t> hap_data, rs_data, q_data, i_data, d_data, c_data;
    create_simple_test_case(tc, hap_data, rs_data, q_data, i_data, d_data, c_data);
    
    double result = compute_pairhmm_avx512_double(tc);
...
void test_precision_consistency()
{
    ...
    float result_avx512_f = compute_pairhmm_avx512_float(tc);
    double result_avx512_d = compute_pairhmm_avx512_double(tc);
    
    printf("  AVX2 Float:    %f\n", result_avx2_f);
    printf("  AVX2 Double:   %lf\n", result_avx2_d);
    printf("  AVX512 Float:  %f\n", result_avx512_f);
    printf("  AVX512 Double: %lf\n", result_avx512_d);
    
    // 检查 float 和 double 结果的相对误差
    double rel_error = fabs(result_avx2_f - result_avx2_d) / fabs(result_avx2_d);
    printf("  Relative error (AVX2): %e\n", rel_error);
...
{
    printf("===========================================\n");
    printf("PairHMM Intra Unit Tests\n");
    printf("===========================================\n\n");
